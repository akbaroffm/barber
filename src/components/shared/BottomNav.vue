<template>
  <nav class="fixed bottom-0 left-0 right-0 z-50 flex" style="background: var(--tg-bg); border-top: 0.5px solid var(--tg-separator);">
    <router-link
      v-for="item in navItems"
      :key="item.path"
      :to="item.path"
      class="flex-1 flex flex-col items-center gap-0.5 py-2 pt-2.5"
      :style="{ color: isActive(item.path) ? 'var(--tg-link)' : 'var(--tg-hint)' }"
    >
